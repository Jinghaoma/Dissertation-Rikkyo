...
final %>% group_by(eta) %>% dplyr::summarise(n=n())

final %>% group_by(eta, r) %>% dplyr::summarise(n=n())

final %>% group_by(eta, r) %>% dplyr::summarise(n=n()) %>% ungroup() %>% select(n) %>% table(.)
